...
import { EmailVerificationRequest, Otp, StatusMessage, User } from '../models/models';
...
export class EmailVerificationService {
  ...
  private userSubject = new BehaviorSubject<User>({email:'',role:'',password:''});
  ...
  verifyEmail(emailVerificationRequest:EmailVerificationRequest):Observable<StatusMessage>{
    this.emailSubject.next(emailVerificationRequest.email)
    const status: Observable<StatusMessage> = this.http.post<any>(
      `${this.baseUrl}email-verification`,emailVerificationRequest
    );
    return status
  }
